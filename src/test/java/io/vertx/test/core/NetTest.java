...
public class NetTest extends VertxTestBase {
  ...
  public void testClientSendBufferWithServerWorkerStarvation() throws Exception {
    ...
    List<Context> contexts = createWorkers(size + 1);
    CountDownLatch latch1 = new CountDownLatch(contexts.size() - 1);
    contexts.get(0).runOnContext(v -> {
      ...
      server.listen(1234, ar -> {
        ...
        for (int i = 1; i < contexts.size(); i++) {
          contexts.get(i).runOnContext(v2 -> {
            latch1.countDown();
            try {
              Thread.sleep(1000);
